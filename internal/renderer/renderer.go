...
func writeSingleSlideHTML(pres *model.Presentation, slideIdx int, allHTMLPath, outPath string, wPx, hPx int) error {
	...
	// Do NOT fix html/body to the viewport size: when html or body carries an
	// explicit height, Chromium's headless screenshot path fails to paint the
	// .slide background (it stays white) once the .stage transform layering
	// kicks in. Letting the page size itself naturally avoids it.
	...
	// Background must live in <head>: a <style> element nested inside a <div>
...
// slideBackgroundCSS returns the bare CSS rule (no <style> wrapper) that
// applies the slide background to .slide.
func slideBackgroundCSS(g *htmlGenerator, slide *model.Slide) string {
	bg := slide.Background
	if bg == nil {
		if g.pres.Theme.BackgroundColor != "" {
			return fmt.Sprintf(".slide { background: %s; }\n", cssColor(g.pres.Theme.BackgroundColor))
		}
		return ""
	}
	switch bg.Type {
	...
	case model.BgImage:
		if uri := g.dataURI(bg.ImagePath); uri != "" {
			return fmt.Sprintf(".slide { background: url(%s) center/cover no-repeat; }\n", uri)
		}
	}
	return ""
}
...
// (used by GenerateHTML, where the rule lands as a body-level <style> child).
func slideBackgroundStyle(g *htmlGenerator, slide *model.Slide) string {
	css := slideBackgroundCSS(g, slide)
	if css == "" {
		return ""
	}
